...
// Cada vez que se la llama devuelve los ultimos pedidos realizados
function getOrders() {
...
// Funcion para listar todos los pedidos en el HTML con su respectiva informacion
export function loadOrders(orders) {
    ...
    if (orders.length != 0) {
        ...
        orders.forEach((order) => {
            ...
            ordersListElement.innerHTML += `
				<div class="w-full flex items-center justify-between p-3 max-tablet:flex-col max-tablet:gap-2 max-tablet:items-start">
					 <div class="flex items-stretch gap-2 h-12 h-full max-tablet:min-h-16">
						  <img class="rounded-xl h-12 max-tablet:h-16" src="${order.image}" alt="${order.title}" />
						  <div class="flex flex-col justify-between">
							<h3 class="text-base">${order.title} x${order.amount} &bull; <span class="text-xs text-[var(--text-color-secondary)]">
								${new Date(order.date).toLocaleString("es-AR")}</span>
							</h3>
					 ...
					 <button
						  id="btn_canceled"
						  data-id="${order.id}"
						  class="h-12 cursor-pointer py-3 px-4 rounded-xl text-[var(--primary-color)] bg-[var(--low-tone-color)] transition-all hover:bg-[var(--primary-color)] hover:text-[var(--text-color)] max-tablet:w-full"
						  >
						  Cancelar
					 </button>
				</div>
    ...
    setTimeout(() => {
        ...
        orders && orders.forEach((order) => {
            if (order.status === "in_process") {
                ...
                const filteredOrders = updateFilters(orders)
...
if (location.pathname.endsWith("pedidos.html")) {
    ...
    filterButtonsElements.forEach((el) => {
        el.addEventListener('click', (e) => {
            ...
            const filteredOrders = updateFilters(getOrders(), { type: filterType, value: filterValue }, true)
            loadOrders(filteredOrders)
        })
    })
