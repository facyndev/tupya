...
async function updateSearch(search) {
    ...
    const filteredFoods = search != "" ? foods.filter(food => food.strMeal.toLowerCase().includes(search.toLowerCase())) : foods; 

    if(filteredFoods.length === 0) {
        productsListElement.innerHTML = '<p class="text-base text-[var(--text-color-secondary)] w-150 line-clamp-2">Upps! No encontramos esa comida.</p>'
    } else {
        ...
        filteredFoods.map((meal) => {
            ...
            const { strMeal: title, strInstructions: description, strMealThumb: image } = meal;
    
            // Filtrar solo las claves del objecto que sean ingredientes
            const ingredients = Object.entries(meal).filter((key) => key[0].includes('strIngredient') && key[1] != "").map((ingredient) => ingredient[1])
                ...
                <div class="w-full flex justify-between">
                    <div class="w-full flex items-start gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            ${ingredients.map((ingredient) => `<img src="https://www.themealdb.com/images/ingredients/${encodeURIComponent(ingredient)}.png" class="rounded-xl" alt="${ingredient}" width="24" height="24"/>`).join("")}
                        </div>
                        </div>
                    </div>
