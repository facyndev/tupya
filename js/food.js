...
if (location.pathname.includes("comida.html")) {
  window.addEventListener("load", () => {
    productDetailsElement.innerHTML =
      '<p class="w-full mx-auto text-center text-xl font-medium text-[var(--text-color-secondary)]">Cargando...</p>';
      ...
      .then((food) => {
        ...
        productDetailsElement.innerHTML = `
                    <div class="flex gap-2 tablet:h-[calc(100vh-95px)] sticky tablet:top-4 max-tablet:static max-tablet:flex-col-reverse">
                        <div class="flex flex-col gap-2 w-16 overflow-y-auto max-tablet:flex-row max-tablet:w-full max-tablet:h-16 max-tablet:overflow-x-auto">
                            ${ingredients
                              .map((ingredient) =>
                                ingredient
                                  ? `<img src="https://www.themealdb.com/images/ingredients/${encodeURI(
                                      ingredient + "-small"
                                    )}.png" class="w-full h-auto max-tablet:w-fit" alt="${ingredient}" title="${ingredient}">`
                                  : ""
                              )
                              .join("")}
                        </div>
                    ...
                    <div class="flex flex-col gap-4 flex-1">
                        ...
                        ${
                          tags !== null
                            ? tags
                                .split(",")
                                .map(
                                  (tag) => `<span class="inline">${tag}</span>`
                                )
                                .join(" &bull; ")
                            : ""
                        }
                        ...
                        <iframe class="w-full rounded-xl" width="560" height="315" src="https://www.youtube.com/embed/${
                          youtubeVideo.split("?v=")[1]
